...
public class Main {
	...
	public static void main(String[] args) {
		...
		executar(usuarios, clientes, empresas, produtos, carrinho, vendas, empresa);
	}

	public static void executar(List<Usuario> usuarios, List<Cliente> clientes, List<Empresa> empresas,
								List<Produto> produtos, List<Produto> carrinho, List<Venda> vendas, Empresa empresa) {
				...
				if (usuarioLogado.IsEmpresa()) {
					...
					switch (escolha) {
						case 1: {
							...
							vendas.stream().forEach(venda -> {
								if (venda.getEmpresa().getId().equals(usuarioLogado.getEmpresa().getId())) {
									System.out.println("************************************************************");
									System.out.println("Venda de codigo: " + venda.getCodigo() + " no CPF "
									...
									BigDecimal totalLiquido = BigDecimal.valueOf(venda.getValor()).subtract(venda.calcularComissaoSistema());
									System.out.println("Total Líquido  para empresa: R$" + df.format(totalLiquido));
									System.out.println("************************************************************");
								}
							});
							if (usuarioLogado.IsAdmin() || (usuarioLogado.IsEmpresa() && usuarioLogado.getEmpresa().getId().equals(empresa.getId()))) {
								System.out.println("Saldo Empresa: " + usuarioLogado.getEmpresa().getSaldo());
							} else {
								System.out.println("Você não tem permissão para visualizar as vendas desta empresa.");
							}
							System.out.println("************************************************************");
						...
						case 2: {
							...
							List<Produto> produtosDaEmpresa = produtos.stream()
									.filter(produto -> produto.getEmpresa().getId().equals(usuarioLogado.getEmpresa().getId()))
									.collect(Collectors.toList());
					...
					case 1: {
						...
						empresas.sort(Comparator.comparingInt(Empresa::getId));
						...
						do {
							...
							List<Produto> produtosDaEmpresa = produtos.stream()
									.filter(x -> x.getEmpresa().getId().equals(escolhaEmpresa))
									.collect(Collectors.toList());
							...
							if (escolhaProduto > 0 && escolhaProduto < contador) {
								int indiceProdutoEscolhido = escolhaProduto - 1; // Subtrai 1 para obter o índice correto
								if (indiceProdutoEscolhido >= 0 && indiceProdutoEscolhido < produtosDaEmpresa.size()) {
									Produto produtoEscolhido = produtosDaEmpresa.get(indiceProdutoEscolhido);
						...
						case 2: {
							if (usuarioLogado.getCliente() != null) {
								...
								for (Venda compra : usuarioLogado.getCliente().getCompras()) {
									System.out.println("************************************************************");
									System.out.println("Compra de código: " + compra.getCodigo() + " na empresa "
											+ compra.getEmpresa().getNome() + ": ");
									System.out.println("Produtos comprados:");

									for (Produto produto : compra.getItens()) {
										System.out.println(produto.getNome()); // Imprime o nome do produto
									}

									System.out.println("************************************************************");
								}
							} else {
								System.out.println("Usuário não tem um cliente associado.");
							}
	...
	public static Venda criarVenda(List<Produto> carrinho, Empresa empresa, Cliente cliente, List<Venda> vendas) {
		// Calcula o valor total dos itens no carrinho usando BigDecimal
		BigDecimal valorTotal = carrinho.stream()
				.map(produto -> BigDecimal.valueOf(produto.getPreco()))
				.reduce(BigDecimal.ZERO, BigDecimal::add);

		int idVenda = obterProximoIdVenda(vendas); 		// Obtém o próximo ID de venda


		BigDecimal comissaoSistema = valorTotal.multiply(BigDecimal.valueOf(empresa.getTaxa())); 		// Calcula a comissão do sistema usando BigDecimal


		valorTotal = valorTotal.setScale(2, RoundingMode.HALF_UP);		// Arredonda os valores para duas casas decimais

		comissaoSistema = comissaoSistema.setScale(2, RoundingMode.HALF_UP);

		Venda venda = new Venda(idVenda, carrinho, valorTotal.doubleValue(), comissaoSistema.doubleValue(), empresa, cliente); 		// Cria a instância de Venda

		cliente.adicionarCompra(venda);


		venda.confirmarVenda(); // Chama o método para confirmar a venda
	...
	private static int obterProximoIdVenda(List<Venda> vendas) {
		return vendas.isEmpty() ? 1 : vendas.get(vendas.size() - 1).getCodigo()+ 1;
	}
}
